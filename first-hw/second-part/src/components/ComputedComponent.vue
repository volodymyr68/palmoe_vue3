...
<template>
  <div class="taskBox">
    <p>
      Створіть обчислювальну властивість для виведення даних, обчислених на
      основі інших реактивних даних.
    </p>
    <h1>{{ fullName }}</h1>
  ...
  <div class="taskBox">
    <p>
      Створіть computed property, яке комбінує дані відповіді від HTTP запиту та
      з рекативною змінною.
    </p>
  ...
  <div class="taskBox">
    <p>
      Використовуйте watcher для відстеження змін у реактивному об’єкті та
      виведення повідомлення про це.
    </p>
    <input
        type="text"
        v-model="this.watcherObject.name"
        @input="this.watcherObject.name = $event.target.value"
    />
    <div>{{watcherResponse}}</div>
  </div>
  <div class="taskBox">
    <p>
      Використовуйте watcher для відстеження змін однієї властивості у
      реактивному об’єкті та виведення повідомлення про це.
    </p>
    <input
        type="text"
        v-model="this.watcherObject.id"
        @input="this.watcherObject.id = $event.target.value"
    />
    <div>{{watcherResponseId}}</div>
  </div>
  <div class="taskBox">
    <p>
      Використовуйте watcher для відправлення API запитів при зміні реактивних
      даних.
    </p>
    <div>
      ...
      <input
          type="checkbox"
          @input="this.makeRequest = $event.target.checked"
      />
    </div>
    <p>{{newResponse}}</p>
  </div>
  ...
  <div class="taskBox">
    <p>
      Використовуйте deep опцію для відстеження властивостей внутрішніх
      об’єктів.
    </p>
    <input
        type="text"
        @input="this.newUser.address.street=$event.target.value"
    />
    <p>{{deepWatcher}}</p>
  </div>
  ...
  <div class="taskBox">
    <p>
      Використовуйте опцію immediate для виклику watcher при ініціалізації
      компонента.
    </p>
    <p>{{deepWatcher}}</p>
  </div>
